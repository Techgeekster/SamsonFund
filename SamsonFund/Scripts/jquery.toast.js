(function($){
	var th = null, cf = null, toast = function(m,o){
		...
		if(!th){ 
			...
			typeof cf.height === 'number' || (cf.height = 200);
			...
			th.width(cf.width);
			th.height(cf.height);
			(cf.align === 'left' || cf.align === 'right') && th.css('margin','5px').css(cf.align, '0') || th.css({left: '50%', margin: (cf.height / 2) + 'px 0 0 -' + (cf.width / 2) + 'px'});
		}
		// create toast
	...
	toast.config = { width: 500, height: 200, align: 'center', closeForStickyOnly: true };
	$.extend({ toast: toast });
})(jQuery);